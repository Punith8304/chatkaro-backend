...
export const getUserFriendsList = async (user) => {
    try {
...
export const changeChatList = async (receiver, sender) => {
    try {
...
export const addToFriendsList = async ({ receiver, sender }) => {
    try {
...
const updateFriendsList = async (user, friend) => {
    try {
...
export const searchFriends = async (searchQuery) => {
    try {
        ...
        return { fetched: true, result: searchFriendsResult.map(user => user.userName) };
    }
    catch (error) {
        console.log(error);
...
export const getUnreadCount = async (user, userFriend) => {
    try {
        const unreadMessageList = await chatModel.aggregate([
        ...
        return { status: true, unreadCount: unreadMessageList[0]?.unread };
    }
    catch (error) {
        console.log(error);
...
export const updateUnReadCount = async (user, userFriend) => {
    try {
        const unReadMsgsCount = await getUnreadCount(user, userFriend);
...
export const suggestedFriendsUsingLimit = async (userName) => {
    try {
...
export const checkUserExisits = async (username) => {
    try {
...
//# sourceMappingURL=friendListServices.js.map